...
import { B as resolveUserPath, O as clampInt, k as clampNumber, q as truncateUtf16Safe, t as createSubsystemLogger } from "./subsystem-loAAXGlc.js";
import { n as retryAsync } from "./retry-4dLTmoU_.js";
import { r as isTruthyEnvValue } from "./command-format-DI57or3f.js";
import { E as resolveApiKeyForProvider, T as requireApiKey } from "./model-selection-BosKk3MZ.js";
import { n as resolveAgentConfig, r as resolveAgentDir, s as resolveAgentWorkspaceDir } from "./agent-scope-CdRMHY-e.js";
import { n as formatErrorMessage } from "./errors-BCGY_vDh.js";
import { t as fetchWithSsrFGuard } from "./fetch-guard-CpixodvE.js";
...
*/
function ensureFactsSchema(params) {
    ...
    `);
		...
		params.db.exec(`CREATE INDEX IF NOT EXISTS idx_facts_current ON facts(valid_to) WHERE valid_to IS NULL;`);
		params.db.exec(`CREATE INDEX IF NOT EXISTS idx_facts_created ON facts(created_at);`);
		params.db.exec(`CREATE INDEX IF NOT EXISTS idx_facts_stale ON facts(last_referenced_at) WHERE valid_to IS NULL;`);
		factsReady = true;
	} catch (err) {
		factsError = err instanceof Error ? err.message : String(err);
		return {
			factsReady,
			factsFtsReady,
			factsError
	...
	try {
		params.db.exec("CREATE VIRTUAL TABLE IF NOT EXISTS facts_fts USING fts5(\n  entity,\n  attribute,\n  value,\n  tags,\n  fact_id UNINDEXED,\n  tokenize='porter'\n);");
		factsFtsReady = true;
	} catch (err) {
		factsError = err instanceof Error ? err.message : String(err);
	}
	...
	*/
	...
	constructor(params) {
		...
		if (params.factsOnly) {
			...
			if (factsResult.factsError) log.warn(`facts schema issue: ${factsResult.factsError}`);
			this.vector = {
				enabled: false,
				available: false
			};
	...
	saveFact(params) {
		...
		const existing = this.db.prepare(`SELECT id, value FROM facts WHERE entity = ? AND attribute = ? AND valid_to IS NULL`).all(entity, attribute);
		...
		try {
			...
			if (existing.length > 0) {
				const invalidateStmt = this.db.prepare(`UPDATE facts SET valid_to = ?, superseded_by = ? WHERE id = ?`);
				for (const old of existing) invalidateStmt.run(now, newId, old.id);
			}
			this.db.prepare(`INSERT INTO facts (id, entity, attribute, value, tags, confidence, valid_from, source, source_conversation_id, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(newId, entity, attribute, value, tags, confidence, now, source, params.sourceConversationId ?? null, now);
			this.db.prepare(`INSERT INTO facts_fts (entity, attribute, value, tags, fact_id) VALUES (?, ?, ?, ?, ?)`).run(entity, attribute, value, tags ?? "", newId);
			this.db.exec("COMMIT");
		} catch (err) {
			try {
				this.db.exec("ROLLBACK");
		...
		return {
			id: newId,
			message: action === "superseded" ? `Saved (superseded ${existing.length} previous fact${existing.length > 1 ? "s" : ""}).` : "Saved.",
			action
		};
	}
	...
	searchFacts(params) {
		...
		const ftsQuery = query.replace(/[":*(){}[\]^~\\]/g, " ").split(/\s+/).filter(Boolean).map((term) => `"${term}"`).join(" ");
		if (!ftsQuery) return {
			results: [],
			count: 0
		};
		const conditions = [];
		const sqlParams = [ftsQuery];
		if (!params.includeHistorical) conditions.push(`f.valid_to IS NULL`);
		if (params.entity) {
		...
		if (params.tags && params.tags.length > 0) {
			...
			for (const tag of params.tags) sqlParams.push(`%${JSON.stringify(tag).slice(1, -1)}%`);
		}
		const whereClause = conditions.length > 0 ? ` AND ${conditions.join(" AND ")}` : "";
		sqlParams.push(limit);
		const sql = `SELECT f.id, f.entity, f.attribute, f.value, f.tags, f.confidence,  f.valid_from, f.valid_to, f.superseded_by, f.source, f.reference_count,  f.last_referenced_at, f.created_at,  bm25(facts_fts) AS rank FROM facts_fts JOIN facts AS f ON facts_fts.fact_id = f.id WHERE facts_fts MATCH ?${whereClause} ORDER BY rank ASC LIMIT ?`;
		...
		const now = (/* @__PURE__ */ new Date()).toISOString();
		const updateStmt = this.db.prepare(`UPDATE facts SET reference_count = reference_count + 1, last_referenced_at = ? WHERE id = ?`);
	...
	getLastConsolidation(type) {
		const row = this.db.prepare(`SELECT id, type, session_id, facts_added, facts_updated, facts_invalidated,
                started_at, completed_at, model_used, tokens_used
         FROM consolidation_log WHERE type = ? ORDER BY started_at DESC LIMIT 1`).get(type);
		return row ? mapConsolidationRow(row) : null;
	}
	/** Write a consolidation log entry. */
	logConsolidation(params) {
		const id = randomUUID();
		this.db.prepare(`INSERT INTO consolidation_log (id, type, session_id, facts_added, facts_updated, facts_invalidated, started_at, completed_at, model_used, tokens_used)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, params.type, params.sessionId ?? null, params.factsAdded ?? 0, params.factsUpdated ?? 0, params.factsInvalidated ?? 0, params.startedAt, params.completedAt ?? null, params.modelUsed ?? null, params.tokensUsed ?? null);
		return id;
	}
	/** Get stats about the fact store. */
	getFactStats() {
		const total = this.db.prepare(`SELECT COUNT(*) AS cnt FROM facts`).get().cnt;
		const current = this.db.prepare(`SELECT COUNT(*) AS cnt FROM facts WHERE valid_to IS NULL`).get().cnt;
		const superseded = total - current;
		const entityRows = this.db.prepare(`SELECT entity, COUNT(*) AS cnt FROM facts WHERE valid_to IS NULL GROUP BY entity ORDER BY cnt DESC LIMIT 20`).all();
		const unreferenced = this.db.prepare(`SELECT COUNT(*) AS cnt FROM facts WHERE valid_to IS NULL AND reference_count = 0`).get().cnt;
		...
		return {
			...
			superseded,
			unreferenced,
			createdToday: this.db.prepare(`SELECT COUNT(*) AS cnt FROM facts WHERE created_at >= ?`).get(todayStart.toISOString()).cnt,
			topEntities: entityRows.map((r) => ({
				entity: r.entity,
				count: r.cnt
			}))
		};
	...
	findDuplicateFacts() {
		const rows = this.db.prepare(`SELECT f.id, f.entity, f.attribute, f.value, f.tags, f.confidence,
         ...
         ORDER BY f.entity, f.attribute, f.created_at DESC`).all();
	...
	applyFactExpiry() {
		...
		const details = [];
		let invalidated = 0;
		const facts = this.db.prepare(`SELECT id, entity, attribute, tags, reference_count, last_referenced_at, created_at
         FROM facts WHERE valid_to IS NULL`).all();
		const invalidateStmt = this.db.prepare(`UPDATE facts SET valid_to = ?, superseded_by = 'expired' WHERE id = ?`);
		...
		for (const fact of facts) {
			const tags = fact.tags ? JSON.parse(fact.tags) : [];
			const ageDays = (now.getTime() - new Date(fact.created_at).getTime()) / (1e3 * 60 * 60 * 24);
			if (tags.some((t) => neverExpireTags.has(t))) continue;
			if (fact.reference_count >= 3) continue;
			let reason = "";
			if (tags.includes("type:debug") || tags.includes("type:error")) {
				if (ageDays > 14 && !tags.includes("root-cause")) reason = "debug/error fact older than 14 days";
			} else if (tags.includes("type:observation")) {
				if (ageDays > 30) reason = "observation older than 30 days without promotion";
			} else if (tags.includes("type:session-context")) {
				if (ageDays > 30) reason = "session-context older than 30 days";
			} else if (tags.includes("type:commitment") || tags.includes("type:todo")) {
				if (ageDays > 90) reason = "commitment/todo older than 90 days";
			} else if (ageDays > 60 && fact.reference_count === 0) reason = "unreferenced fact older than 60 days";
			if (reason) toExpire.push({
				id: fact.id,
				entity: fact.entity,
	...
	getRecentFacts(params) {
		...
		let sql = "SELECT id, entity, attribute, value, tags, confidence, valid_from, valid_to, superseded_by, source, reference_count, last_referenced_at, created_at FROM facts WHERE valid_to IS NULL";
		const sqlParams = [];
		if (params?.since) {
			sql += ` AND created_at >= ?`;
		...
		sqlParams.push(limit);
		return this.db.prepare(sql).all(...sqlParams).map((row) => ({
			id: row.id,
			entity: row.entity,
			attribute: row.attribute,
			value: row.value,
...
function mapConsolidationRow(row) {
	return {
		id: row.id,
		type: row.type,
...
//#endregion
export { manager_exports as n, resolveMemorySearchConfig as r, MemoryIndexManager as t };