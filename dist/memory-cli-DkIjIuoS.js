...
import { T as parseDurationMs, i as loadConfig, mt as splitShellArgs } from "./config-8-je4WFT.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-FVP3UopW.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-ClbElkDE.js").then((n) => n.n);
