import { X as resolveStateDir, pt as resolveRequiredHomeDir } from "./entry.js";
import { _ as isSubagentSessionKey, l as normalizeAgentId, n as DEFAULT_AGENT_ID, v as parseAgentSessionKey } from "./session-key-BRdRr5Ah.js";
...
function validateExtraBootstrapPath(name) {
	return name.endsWith(".md") && !path.isAbsolute(name) && !name.includes("..") && !name.includes("/") && name.length > 0 && name.length <= 64;
}
async function loadWorkspaceBootstrapFiles(dir, extraFiles) {
	...
	if (extraFiles) {
		const validated = extraFiles.slice(0, MAX_EXTRA_BOOTSTRAP_FILES).filter(validateExtraBootstrapPath);
		for (const name of validated) entries.push({
			name,
