...
import { u as installUnhandledRejectionHandler } from "./runner-DeOMXCdu.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-BG8E1dle.js";
import { t as assertSupportedRuntime } from "./runtime-guard-D09-7j7u.js";
import { t as emitCliBanner } from "./banner-1TkdYW49.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-DsDZnXAK.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-CKe3MAuZ.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-BiQRsyWE.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-DkIjIuoS.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-BpsCZiRr.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-BpsCZiRr.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-Be1y9vcB.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-Be1y9vcB.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-DhjCGtfZ.js");
	...
	if (primary) {
		...
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-DAi0V_4N.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-niOAR97A.js").then((n) => n.a);
