...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-xzmW8OWl.js";
import { t as assertSupportedRuntime } from "./runtime-guard-D09-7j7u.js";
import { t as emitCliBanner } from "./banner-CfQPouWh.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-C3hJk35j.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BifZPjoS.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-CSJ3Fp_d.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-C8PU7FWZ.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-C8PU7FWZ.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-CUymPSlv.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-CUymPSlv.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-BLpnwStt.js");
	...
	if (primary) {
		...
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-DD5cA9EG.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-zamrRqMU.js").then((n) => n.a);
